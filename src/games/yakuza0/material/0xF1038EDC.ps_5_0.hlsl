#include "../common.hlsl"

// ---- Created with 3Dmigoto v1.3.16 on Sat Oct 11 15:29:07 2025
Texture2D<float4> t14 : register(t14);

Texture2D<float4> t13 : register(t13);

Texture2D<float4> t5 : register(t5);

Texture2D<float4> t3 : register(t3);

Texture2D<float4> t0 : register(t0);

SamplerState s14_s : register(s14);

SamplerState s13_s : register(s13);

SamplerState s5_s : register(s5);

SamplerState s3_s : register(s3);

SamplerState s0_s : register(s0);

cbuffer cb3 : register(b3)
{
  float4 cb3[11];
}

cbuffer cb2 : register(b2)
{
  float4 cb2[92];
}

cbuffer cb1 : register(b1)
{
  float4 cb1[13];
}

cbuffer cb9 : register(b9)
{
  float4 cb9[16];
}

cbuffer cb5 : register(b5)
{
  float4 cb5[2];
}

cbuffer cb6 : register(b6)
{
  float4 cb6[2];
}

cbuffer cb7 : register(b7)
{
  float4 cb7[11];
}

cbuffer cb13 : register(b13)
{
  float4 cb13[1];
}

cbuffer cb12 : register(b12)
{
  float4 cb12[30];
}




// 3Dmigoto declarations
#define cmp -


void main(
  float4 v0 : COLOR0,
  float4 v1 : COLOR1,
  float3 v2 : TEXCOORD0,
  float w2 : TEXCOORD6,
  float4 v3 : TEXCOORD1,
  float4 v4 : TEXCOORD2,
  float4 v5 : TEXCOORD3,
  float4 v6 : TEXCOORD4,
  float4 v7 : TEXCOORD5,
  float4 v8 : SV_POSITION0,
  out float4 o0 : SV_Target0)
{
  float4 r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,r14,r15;
  uint4 bitmask, uiDest;
  float4 fDest;

  r0.xyzw = t0.Sample(s0_s, v2.xy).xyzw;
  r0.xyz = r0.xyz * r0.xyz;
  r1.xyz = t3.Sample(s3_s, v2.xy).xyz;
  r2.xy = t5.Sample(s5_s, v2.xy).xy;
  r2.xy = r2.xy * float2(2,2) + float2(-1,-1);
  r1.w = dot(r2.xy, r2.xy);
  r1.w = 1 + -r1.w;
  r1.w = max(0, r1.w);
  r1.w = sqrt(r1.w);
  r3.xyz = v5.yzx * v4.zxy;
  r3.xyz = v4.yzx * v5.zxy + -r3.xyz;
  r3.xyz = v5.www * r3.xyz;
  r2.xzw = v5.xyz * r2.xxx;
  r2.xyz = r2.yyy * r3.xyz + r2.xzw;
  r2.xyz = r1.www * v4.xyz + r2.xyz;
  r1.w = dot(r2.xyz, r2.xyz);
  r1.w = rsqrt(r1.w);
  r3.xyz = r2.xyz * r1.www;
  r2.w = dot(v3.xyz, v3.xyz);
  r2.w = rsqrt(r2.w);
  r4.xyz = v3.xyz * r2.www;
  r5.xyz = v3.xyz * r2.www + cb3[6].xyz;
  r4.w = dot(r5.xyz, r5.xyz);
  r4.w = rsqrt(r4.w);
  r5.xyz = r5.xyz * r4.www;
  r6.xyzw = r2.yxyz * r1.wwww + float4(0.100000001,1,1,1);
  r6.x = saturate(r6.x);
  r2.xy = cb7[6].wz + float2(2,2.5);
  r1.w = dot(-r4.xyz, v4.xyz);
  r1.w = saturate(1 + -r1.w);
  r2.z = dot(-r4.xyz, r3.xyz);
  r2.z = 1 + -r2.z;
  r4.w = saturate(r2.z);
  r1.w = r4.w * r1.w;
  r1.w = log2(r1.w);
  r2.x = r2.x * r1.w;
  r2.x = exp2(r2.x);
  r2.x = r2.x * r2.y;
  r2.x = r2.x * r6.x;
  r7.xyzw = int4(4096,8,1,0x4000) & asint(cb13[0].xxxx);
  r1.w = cb9[9].w * r1.w;
  r1.w = exp2(r1.w);
  r1.w = cb9[9].z * r1.w;
  r1.w = r1.w * r6.x;
  r1.w = r7.x ? r1.w : r2.x;
  r1.w = r1.w * cb9[2].x + 1;
  r0.xyz = r1.www * r0.xyz;
  r2.xy = cb12[29].zw * v8.xy;
  if (r7.y != 0) {
    r8.xy = t13.Sample(s13_s, r2.xy).xy;
  } else {
    r8.xy = float2(1,1);
  }
  r8.zw = r8.xy * cb1[12].xz + cb1[12].yw;
  r8.xy = r7.yy ? r8.zw : r8.xy;
  r1.w = min(r8.x, r8.y);
  r9.xyz = v3.xyz * r2.www + cb3[7].xyz;
  r4.w = dot(r9.xyz, r9.xyz);
  r4.w = rsqrt(r4.w);
  r9.xyz = r9.xyz * r4.www;
  r4.w = dot(r4.xyz, r3.xyz);
  r4.w = r4.w + r4.w;
  r10.xyz = r3.xyz * -r4.www + r4.xyz;
  r11.x = saturate(dot(r10.xyz, -cb3[6].xyz));
  r11.y = saturate(dot(r10.xyz, -cb3[7].xyz));
  r8.zw = log2(r11.xy);
  r8.zw = cb7[10].ww * r8.zw;
  r8.zw = exp2(r8.zw);
  r4.w = cb1[0].w * cb7[8].z;
  r5.w = cb7[10].w * 0.0397435986 + 0.0856831968;
  r8.zw = r8.zw * r5.ww;
  r5.x = dot(r4.xyz, r5.xyz);
  r5.y = dot(r4.xyz, r9.xyz);
  r4.xy = float2(1,1) + -r5.xy;
  r5.xy = r4.xy * r4.xy;
  r4.z = -cb7[8].z * cb1[0].w + 1;
  r5.xy = r5.xy * r5.xy;
  r5.xy = r5.xy * r4.zz;
  r4.xy = r5.xy * r4.xy + r4.ww;
  r4.xy = r8.zw * r4.xy;
  r5.x = dot(r3.xyz, -cb3[6].xyz);
  r4.z = saturate(r5.x);
  r4.x = r4.x * r4.z;
  r5.y = dot(r3.xyz, -cb3[7].xyz);
  r5.xyz = saturate(r5.yxy);
  r4.y = r5.x * r4.y;
  r9.xyz = cb6[1].xyz * cb7[10].xyz;
  r4.xzw = cb3[0].xyz * r4.xxx;
  r10.xyz = cb3[1].xyz * r4.yyy;
  r4.xyz = r4.xzw * r1.www + r10.xyz;
  r4.w = -0.5 + w2.x;
  r5.x = cmp(r4.w < 0);
  r5.xw = r5.xx ? float2(1,30) : 0;
  r4.w = r5.w + r4.w;
  r4.w = -1 + r4.w;
  r5.w = cmp(r4.w < 0);
  r8.zw = r5.ww ? float2(1,30) : 0;
  r10.xyzw = cb2[1].xyzw * r8.zzzz;
  r10.xyzw = cb2[0].xyzw * r5.xxxx + r10.xyzw;
  r4.w = r8.w + r4.w;
  r4.w = -1 + r4.w;
  r5.x = cmp(r4.w < 0);
  r5.xw = r5.xx ? float2(1,30) : 0;
  r10.xyzw = cb2[2].xyzw * r5.xxxx + r10.xyzw;
  r4.w = r5.w + r4.w;
  r4.w = -1 + r4.w;
  r5.x = cmp(r4.w < 0);
  r5.xw = r5.xx ? float2(1,30) : 0;
  r10.xyzw = cb2[3].xyzw * r5.xxxx + r10.xyzw;
  r4.w = r5.w + r4.w;
  r4.w = -1 + r4.w;
  r5.x = cmp(r4.w < 0);
  r5.xw = r5.xx ? float2(1,30) : 0;
  r10.xyzw = cb2[4].xyzw * r5.xxxx + r10.xyzw;
  r4.w = r5.w + r4.w;
  r4.w = -1 + r4.w;
  r5.x = cmp(r4.w < 0);
  r5.xw = r5.xx ? float2(1,30) : 0;
  r10.xyzw = cb2[5].xyzw * r5.xxxx + r10.xyzw;
  r4.w = r5.w + r4.w;
  r4.w = -1 + r4.w;
  r5.x = cmp(r4.w < 0);
  r5.xw = r5.xx ? float2(1,30) : 0;
  r10.xyzw = cb2[6].xyzw * r5.xxxx + r10.xyzw;
  r4.w = r5.w + r4.w;
  r4.w = -1 + r4.w;
  r5.x = cmp(r4.w < 0);
  r5.xw = r5.xx ? float2(1,30) : 0;
  r10.xyzw = cb2[7].xyzw * r5.xxxx + r10.xyzw;
  r4.w = r5.w + r4.w;
  r4.w = -1 + r4.w;
  r5.x = cmp(r4.w < 0);
  r5.xw = r5.xx ? float2(1,30) : 0;
  r10.xyzw = cb2[8].xyzw * r5.xxxx + r10.xyzw;
  r4.w = r5.w + r4.w;
  r4.w = -1 + r4.w;
  r4.w = cmp(r4.w < 0);
  r4.w = r4.w ? 1.000000 : 0;
  r10.xyzw = cb2[9].xyzw * r4.wwww + r10.xyzw;
  r4.w = max(r0.x, r0.y);
  r4.w = max(r4.w, r0.z);
  r0.x = min(r0.x, r0.y);
  r0.x = min(r0.x, r0.z);
  r0.x = r0.x + r4.w;
  r0.y = 0.5 * r0.x;
  r0.z = r10.z + r10.z;
  r4.w = r10.z * 2 + -1;
  r5.x = cmp(0 < r4.w);
  r5.w = r5.x ? 1.000000 : 0;
  r0.x = -r0.x * 0.5 + 1;
  r4.w = 1 + -r4.w;
  r0.x = -r0.x * r4.w + 1;
  r0.y = r0.y * r0.z;
  r0.z = r5.x ? 0 : 1;
  r0.y = r0.y * r0.z;
  r0.xyz = r0.xxx * r5.www + r0.yyy;
  r4.w = cmp(0 < r10.y);
  if (r4.w != 0) {
    r4.w = cmp(r0.z < 0.5);
    r11.xyz = float3(1,0.333333343,-0.333333343) + r10.yxx;
    r5.x = r11.x * r0.z;
    r5.w = r0.z + r10.y;
    r5.w = -r10.y * r0.z + r5.w;
    r4.w = r4.w ? r5.x : r5.w;
    r5.x = r0.z * 2 + -r4.w;
    r8.zw = frac(r11.yz);
    r5.w = -r5.x + r4.w;
    r6.x = 6 * r5.w;
    r10.yz = r6.xx * r8.zw + r5.xx;
    r11.xyzw = cmp(r8.zzzw < float4(0.166666672,0.5,0.666666687,0.166666672));
    r12.xy = float2(0.666666687,0.666666687) + -r8.zw;
    r12.xy = r12.xy * r5.ww;
    r12.xy = r12.xy * float2(6,6) + r5.xx;
    r7.x = r11.z ? r12.x : r5.x;
    r7.x = r11.y ? r4.w : r7.x;
    r0.x = r11.x ? r10.y : r7.x;
    r7.x = frac(r10.x);
    r6.x = r6.x * r7.x + r5.x;
    r11.xyz = cmp(r7.xxx < float3(0.166666672,0.5,0.666666687));
    r7.x = 0.666666687 + -r7.x;
    r5.w = r7.x * r5.w;
    r5.w = r5.w * 6 + r5.x;
    r5.w = r11.z ? r5.w : r5.x;
    r5.w = r11.y ? r4.w : r5.w;
    r0.y = r11.x ? r6.x : r5.w;
    r8.zw = cmp(r8.ww < float2(0.5,0.666666687));
    r5.x = r8.w ? r12.y : r5.x;
    r4.w = r8.z ? r4.w : r5.x;
    r0.z = r11.w ? r10.z : r4.w;
  }
  r4.w = saturate(dot(r3.xyz, -v6.xyz));
  r10.x = v3.w;
  r10.y = v4.w;
  r10.z = v6.w;
  r11.xyz = r10.xyz * r4.www;
  r11.xyz = max(float3(0,0,0), r11.xyz);
  r4.w = saturate(dot(r3.xyz, -v7.xyz));
  r12.x = v0.w;
  r12.y = v1.w;
  r12.z = v7.w;
  r13.xyz = r12.xyz * r4.www;
  r13.xyz = max(float3(0,0,0), r13.xyz);
  r11.xyz = r13.xyz + r11.xyz;
  r13.xyz = v3.xyz * r2.www + v6.xyz;
  r4.w = dot(r13.xyz, r13.xyz);
  r4.w = rsqrt(r4.w);
  r13.xyz = r13.xyz * r4.www;
  r14.xyz = v3.xyz * r2.www + v7.xyz;
  r2.w = dot(r14.xyz, r14.xyz);
  r2.w = rsqrt(r2.w);
  r14.xyz = r14.xyz * r2.www;
  r13.x = saturate(dot(-r13.xyz, r3.xyz));
  r13.y = saturate(dot(-r14.xyz, r3.xyz));
  r5.xw = log2(r13.xy);
  r5.xw = cb7[10].ww * r5.xw;
  r5.xw = exp2(r5.xw);
  r12.xyz = r12.xyz * r5.www;
  r10.xyz = r10.xyz * r5.xxx + r12.xyz;
  r10.xyz = max(float3(0,0,0), r10.xyz);
  r10.xyz = r10.xyz * r9.xyz;
  r10.xyz = min(float3(1,1,1), r10.xyz);
  r11.xyz = r7.zzz ? r11.xyz : float3(0,0,0);
  r10.xyz = r7.zzz ? r10.xyz : float3(0,0,0);
  r12.xyzw = t14.Sample(s14_s, r2.xy).xyzw;
  r13.xyzw = float4(4,4,4,4) * r12.xyzw;
  r14.xyzw = int4(8192,0x8000,16,4) & asint(cb13[0].xxxx);
  r15.xyzw = r12.xyzw + r12.xyzw;
  r12.xyzw = r12.xyzw / cb1[0].zzzz;
  r12.xyzw = r14.xxxx ? r15.xyzw : r12.xyzw;
  r12.xyzw = r7.wwww ? r13.xyzw : r12.xyzw;
  r2.xyw = r12.xyz + r11.xyz;
  r7.xzw = r12.www + r10.xyz;
  r3.w = 1;
  r11.x = dot(cb2[80].xyzw, r3.xyzw);
  r11.y = dot(cb2[81].xyzw, r3.xyzw);
  r11.z = dot(cb2[82].xyzw, r3.xyzw);
  r11.w = dot(cb2[83].xyzw, r3.xyzw);
  r10.x = dot(r3.xyzw, r11.xyzw);
  r11.x = dot(cb2[84].xyzw, r3.xyzw);
  r11.y = dot(cb2[85].xyzw, r3.xyzw);
  r11.z = dot(cb2[86].xyzw, r3.xyzw);
  r11.w = dot(cb2[87].xyzw, r3.xyzw);
  r10.y = dot(r3.xyzw, r11.xyzw);
  r11.x = dot(cb2[88].xyzw, r3.xyzw);
  r11.y = dot(cb2[89].xyzw, r3.xyzw);
  r11.z = dot(cb2[90].xyzw, r3.xyzw);
  r11.w = dot(cb2[91].xyzw, r3.xyzw);
  r10.z = dot(r3.xyzw, r11.xyzw);
  r3.xzw = max(float3(0.0625,0.0625,0.0625), r10.xyz);
  r3.xzw = cb2[79].xxx * r3.xzw + v2.zzz;
  r3.y = r3.y * 0.5 + 0.5;
  r10.xyz = -cb3[10].xyz + cb3[4].xyz;
  r10.xyz = r3.yyy * r10.xyz + cb3[10].xyz;
  r3.xyz = r10.xyz + r3.xzw;
  r5.xyw = cb3[0].xyz * r5.yyy;
  r10.xyz = cb3[1].xyz * r5.zzz;
  r11.xyz = r5.xyw * r1.www + r10.xyz;
  r11.xyz = r11.xyz + r3.xyz;
  r11.xyz = saturate(r11.xyz + r2.xyw);
  r4.xyz = r9.xyz * r4.xyz + r7.xzw;
  r6.xyz = saturate(float3(0.5,0.5,0.5) * r6.yzw);
  r6.xyz = cb9[11].yyy * r6.xyz;
  r6.xyz = cb9[11].xxx * r0.xyz + r6.xyz;
  r12.xyz = cb9[11].zzz * r1.xyz;
  r6.xyz = r12.xyz * r9.xyz + r6.xyz;
  r6.xyz = cb9[11].www * r4.xxx + r6.xyz;
  r6.xyz = cb9[12].xxx * r8.xxx + r6.xyz;
  r6.xyz = cb9[12].yyy * r8.yyy + r6.xyz;
  r8.xyz = cb6[0].xyz * cb7[9].xyz;
  r6.xyz = cb9[13].xxx * r8.xyz + r6.xyz;
  r6.xyz = cb9[13].yyy * r9.xyz + r6.xyz;
  r1.w = r2.z * r2.z;
  r1.w = r1.w * r1.w;
  r1.w = r1.w * r2.z;
  r1.w = r1.w * 0.972222209 + 0.027777778;
  r6.xyz = cb9[13].zzz * r1.www + r6.xyz;
  r3.xyz = cb9[13].www * r3.xyz + r6.xyz;
  r6.xyz = cb9[14].yyy * r8.xyz;
  r3.xyz = r6.xyz * r5.xyw + r3.xyz;
  r5.xyz = cb9[14].zzz * r8.xyz;
  r3.xyz = r5.xyz * r10.xyz + r3.xyz;
  r2.xyz = cb9[14].www * r2.xyw + r3.xyz;
  r3.xyz = cb9[15].xxx * r9.xyz;
  r2.xyz = r3.xyz * r4.xyz + r2.xyz;
  r3.xyz = cb9[15].yyy * r9.xyz;
  r2.xyz = r3.xyz * r4.xyz + r2.xyz;
  r2.xyz = saturate(cb9[15].zzz * r7.xzw + r2.xyz);
  r0.xyz = r11.xyz * r0.xyz;
  r4.xyz = saturate(r4.xyz);
  r1.xyz = r4.xyz * r1.xyz;
  r0.xyz = r0.xyz * r8.xyz + r1.xyz;
  r0.xyz = r14.yyy ? r2.xyz : r0.xyz;
  r1.x = cb5[1].w * cb7[9].w;
  r1.x = cb6[0].w * r1.x;
  r0.w = r1.x * r0.w;
  r1.w = r0.w * r10.w;
  r0.xyz = v0.xyz * r0.xyz;
  r2.xyz = cb1[0].xxx * r0.xyz;

  MaterialPreTonemap(r2.xyz, cb1[0].x);

  r3.xyz = log2(r2.xyz);
  r3.xyz = cb1[0].yyy * r3.xyz;
  r3.xyz = exp2(r3.xyz);
  r4.xyz = r2.xyz + r2.xyz;
  r5.xyz = r2.xyz * float3(0.300000012,0.300000012,0.300000012) + float3(0.0500000007,0.0500000007,0.0500000007);
  r5.xyz = r4.xyz * r5.xyz + float3(0.00400000019,0.00400000019,0.00400000019);
  r2.xyz = r2.xyz * float3(0.300000012,0.300000012,0.300000012) + float3(0.5,0.5,0.5);
  r2.xyz = r4.xyz * r2.xyz + float3(0.0599999987,0.0599999987,0.0599999987);
  r2.xyz = r5.xyz / r2.xyz;
  r2.xyz = float3(-0.0666666701,-0.0666666701,-0.0666666701) + r2.xyz;
  r2.xyz = saturate(float3(1.37906432,1.37906432,1.37906432) * r2.xyz);

  MaterialPostTonemap(r2.xyz);

  r2.xyz = sqrt(r2.xyz);
  r0.xyz = r14.yyy ? r0.xyz : r2.xyz;
  r0.xyz = r14.zzz ? r3.xyz : r0.xyz;
  r2.xyz = v1.xyz + r0.xyz;
  r0.w = saturate(v1.x);
  r3.xyz = cb1[1].xyz + -r0.xyz;
  r0.xyz = r0.www * r3.xyz + r0.xyz;
  r0.xyz = r14.www ? r2.xyz : r0.xyz;
  r2.xyz = cb9[12].zzz * cb9[3].xyz + r0.xyz;
  r2.xyz = cb9[12].www * v0.xyz + r2.xyz;
  r1.xyz = r14.yyy ? r2.xyz : r0.xyz;
  r0.xyzw = cb1[3].xyzw * r1.xyzw;
  o0.xyzw = r7.yyyy ? r1.xyzw : r0.xyzw;
  return;
}